...
class InitProxy
{
    ...
    public void LoginInProxy()
    {
        ...
        Sys.GetFacade().RegisterProxy(new ImageControlProxy());//图片管理模块   
        Sys.GetFacade().RegisterProxy(new InputManagerProxy());//输入管理模块   
        Sys.GetFacade().RegisterProxy(new SyncNotifyContorlProxy());//输入管理模块   
    }
}
